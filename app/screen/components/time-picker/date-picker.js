...
export const DatePicker = ({
  ...
  const submit = () => {
    const values =
      pad(year, 2) +
      '/' +
      pad(monthList.indexOf(month) + 1, 2) +
      '/' +
      pad(day, 2);
    callback(values);
    _close?.();
  ...
  return (
    <AnimatedPressable
      ...
      style={[
        styles.container,
        {bottom, opacity, backgroundColor: backdropColor},
      ]}>
      <View
        style={[
          styles.pickerContainer,
          {backgroundColor: pickerBgColor, borderColor: backdropColor},
        ]}>
        ...
        <View style={{flex: 1, flexDirection: 'row'}}>
          {isLoad && (
            <>
              <Picker
                {...{values: dayList, defaultValue: 5}}
                setValue={setDay}
              />
              <Picker
                {...{values: monthList, defaultValue: 5}}
                setValue={setMonth}
              />
              <Picker
                {...{values: yearList, defaultValue: 5}}
                setValue={setYear}
              />
            </>
          )}
        ...
        <View style={styles.confirmSection}>
          <Pressable style={confirmButtonStyle} onPress={submit}>
            <Text style={textButtonStyle}>{confirmText}</Text>
          </Pressable>
        </View>
      </View>
    </AnimatedPressable>
